...
def construir_arbol_desde_frecuencias(frecuencias):
    ...
    while len(heap) > 1:
        ...
        # CORRECCIÓN: Mismo orden que en codificación
        padre = NodoHuffman(None, nodo1.frecuencia + nodo2.frecuencia)
        padre.izquierda = nodo1  # Siempre nodo1 a izquierda
        padre.derecha = nodo2    # Siempre nodo2 a derecha
...
def convertir_a_arbol_visual(nodo_huffman):
    ...
    # Formatea el contenido del nodo para visualización
    if nodo_huffman.caracter:
        contenido = f"'{nodo_huffman.caracter}':{nodo_huffman.frecuencia}"  # Ej: 'A':5
    else:
        contenido = f"*:{nodo_huffman.frecuencia}"  # Ej: *:10 (nodo interno)
    
    # Crea el nodo visual con el formato correcto
    nodo_visual = Arbol(contenido)
